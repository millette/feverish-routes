...
const wantedImage = (request) => {
  ...
  for (r in request.pre.student.body._attachments) {
    imgIds[r.split('.').slice(0, -1).join('.')] = r
  }
  request.pre.exercices.body.rows = request.pre.exercices.body.rows
    .map((row) => {
      if (imgIds[row.id]) { row.image = imgIds[row.id] }
      return row
    })
}
...
const wantedImage = (request) => {
  let r
  const ex = request.pre.exercices.body.rows.map((row) => row.id)
  // const stu = {}
  for (r in request.pre.student.body._attachments) {
    // if (r.indexOf()
    // request.pre.student.body._attachments[r]
  }
}
*/
...
const resultats = function (request, reply) {
  ...
  // console.log('FULLBODY:', JSON.stringify(body, null, ' '))
...
const studentUser = function (request, reply) {
  nano(process.env.DBURL).auth(process.env.DBUSER, process.env.DBPW, (err, body, headers) => {
    if (err) { return reply(err) }
    nano({ url: dbUsers, cookie: headers['set-cookie'] })
      .get('org.couchdb.user:' + request.auth.credentials.name, (err, body, head) => {
        if (err) { return reply(err) }
...
const studentImage = function (request, reply) {
  nano(process.env.DBURL).auth(process.env.DBUSER, process.env.DBPW, (err, body, headers) => {
    if (err) { return reply(err) }
    nano({ url: dbUsers, cookie: headers['set-cookie'] })
      .attachment.get('org.couchdb.user:' + request.auth.credentials.name, request.params.att, (e, b, h) => {
        if (e) { return reply(e) }
        const img = sharp(b).resize(100).toFormat(h['content-type'].split('/')[1])
        reply(img).type(h['content-type']).etag(h.etag + request.auth.credentials.name)
        // reply(img).type('image/jpeg').etag(h.etag + request.auth.credentials.name)
        // reply(b).type(h['content-type']).etag(h.etag + request.auth.credentials.name)
      })
  })
}
