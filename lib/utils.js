...
const scorePost = function (request, reply) {
  nano(process.env.DBURL).auth(process.env.DBUSER, process.env.DBPW, (err, body, headers) => {
    ...
    const attname = request.params.ex + '.' + (request.payload.jpeg.hapi.filename.split('.').pop())
    const contenttype = request.payload.jpeg.hapi.headers['content-type']
    const pars = { rev: request.pre.student.body._rev }
    request.payload.jpeg.pipe(
      nano({ url: dbUsers, cookie: headers['set-cookie'] })
        .attachment.insert(docname, attname, null, contenttype, pars, (e, b, h) => {
          if (e) { return reply(e) }
