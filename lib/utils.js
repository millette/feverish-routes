  ...
  sharp(request.payload.jpeg).metadata((er1, m) => {
    ...
    nano(process.env.DBURL).auth(process.env.DBUSER, process.env.DBPW, (err, body, headers) => {
      ...
      udb.attachment.insert(docname, attname, request.payload.jpeg, type, params, (e) => {
        if (e) { return reply(e) }
        const exDoc = request.pre.score.body.doc
...
const correctionsUserPost = function (request, reply) {
  ...
  nano(process.env.DBURL).auth(process.env.DBUSER, process.env.DBPW, (err, body, headers) => {
    ...
    // console.log('score atts:', request.params.ex, doc.opaque, doc._rev, request.pre.score.body.doc._attachments)

    nano({ url: dbUsers, cookie: headers['set-cookie'] })
      .insert(doc, (e, b, h) => {
        if (e) { return reply(e) }
        // TODO RYM
        // if (doc.corrections[request.params.ex].reference) {
        if (request.payload.reference) {
          // copy reference image from user doc to exercice doc
          console.log('copy ref img if available')
          gotoNextCorrection(request, reply)
        } else {
          console.log('remove ref img if available')
          nano({ url: dbUrl, cookie: headers['set-cookie'] })
            .attachment.destroy(request.params.ex, doc.opaque + '.jpg', { rev: request.pre.score.body.doc._rev }, (err, body) => {
