...
const score = function (request, reply) {
  ...
  if (!request.pre.student.body.corrections[request.pre.score.body.doc._id]) {
    return reply('no score yet')
  }
...
const studentIndex = (n, request) => {
  return request.pre.students.body.rows.findIndex((el, i) => {
    console.log('n:', i, el.doc.name)
    return el.doc.name === n
  })
}
...
const correctionsUserPost = function (request, reply) {
  ...
  if (request.payload.reference) {
    doc.corrections[request.params.ex].reference = doc.opaque
  }
...
*/
  ...
  console.log('DOC:', JSON.stringify(request.pre.students.body.rows[index].doc, null, ' '))
  // correction[request.params.ex] = { }

/*
  console.log('PL:', JSON.stringify(request.payload, null, ' '))
  console.log('PRE:', JSON.stringify(request.pre, null, ' '))
  console.log('PARAMS:', JSON.stringify(request.params, null, ' '))
*/
  // console.log('DOC:', JSON.stringify(doc, null, ' '))
  /* RYM
   * 1. Store new score for user
   * 2. Redirect to "next" user scoring page
   */
  reply.redirect('/')
  /*
